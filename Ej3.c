...
// Solo para pruebas
void printMatrix(int* M, int n, int origen){
    printf("%d - ", origen);
    for(int i = 0; i < n; i++)
	printf("%d ", M[i]);
    printf("\n");
...
int main(int argc, char *argv[])
{
    ...
    if(mpi_id == 0){
        // Creamos las matrices A y B, C (producto), A2 (intermedia)
	...
	for(int i = 0; i <= 1; i++){
	    for(int j = 0; j <= 1; j++){
		if(2*i+j != 0){
        	    MPI_Recv(A2, n*n/4, MPI_INT, 2*i+j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		    for(int i2 = 0; i2 < n/2; i2++){
		    	for(int j2 = 0; j2 < n/2; j2++){
			    C[j*n*n/2 + i*n/2 + j2 + i2*n] = A2[i2*n/2 + j2];
		    	}
		    }
		}
	    }
	...
	//printMatrix(C, n*n, 4);
        ...
        prodMat0(A, B, C, n); // Funcion ad-hoc, no es una multiplicacion general
